...
if(isset($_SESSION["cart"])){
    if(isset($_POST["P_ID"])){
        ...
        $_SESSION['cart'] = $_SESSION['cart'] + 5;//$_POST["quant"];
...
<div id="content">
    <div id="text1">
        ...
        while($row = mysql_fetch_object($ergebnis))
           ...
           echo "$row->P_ID";
           echo '"></input>';
           echo '</form><form method="POST" action="product_view.php?ProdId=' . $row->P_ID . '"><input type="hidden" name="P_ID" value="' . $row->P_ID . '"></input><input type="text" class="quantity" name="quant" value="1"></input><button type="Submit" class="buy" onClick="onbuy()" name="buy"><div class="buybutton">Buy</div></button></form></div>';
           }
        ?>    
    </div>
</div>
